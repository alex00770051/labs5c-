...
void BinaryProcessor::DisplayBinaryContent(const std::string& filename) {
    ...
    std::cout << "\nДвоичное содержимое файла '" << final_filename << "':"
        << std::endl;
    std::cout << "Размер: " << file_size << " байт" << std::endl;
...
void BinaryProcessor::Task2MatrixProcessing(const std::string& filename) {
    ...
    std::cout << "Создание матрицы " << n << "x" << n << " из "
        << numbers.size() << " чисел" << std::endl;
...
void BinaryProcessor::FillWithRandomLuggage(const std::string& filename,
    ...
    std::vector<std::string> luggage_types = { "чемодан", "сумка", "коробка",
                                              "рюкзак", "портфель", "кофр",
                                              "чемоданчик", "сумка спортивная",
                                              "корзина", "кейс" };
...
void BinaryProcessor::DisplayLuggage(const std::string& filename) {
    ...
    for (size_t i = 0; i < luggage_list.size(); ++i) {
        std::cout << std::left << std::setw(5) << i + 1 << std::setw(25)
            << luggage_list[i].item_name << std::fixed
            << std::setprecision(2) << luggage_list[i].weight << std::endl;
...
void BinaryProcessor::FillTextFileWithRandomNumbersMultiplePerLine(
    ...
    while (numbers_written < total_count) {
        int numbers_in_this_line =
            std::min(numbers_per_line, total_count - numbers_written);
...
void BinaryProcessor::FillTextFileWithRandomStrings(const std::string& filename,
    ...
    std::vector<std::string> sample_words = {
        "дом",   "кот",   "стол", "окно",    "книга",    "ручка",
        "солнце", "ветер", "дождь", "снег",   "город",   "улица",
        "машина", "цветок", "музыка", "компьютер", "программа", "алгоритм",
        "данные", "файл" };
...
void BinaryProcessor::CreateTestTextFileMultiplePerLine(
    ...
    std::vector<std::vector<int>> test_data = {
        {15, -3, 42, 7}, {8, 0, -15, 23}, {100, -50, 33, 17},
        {5, 88, -7, 42}, {19, 21, 100, 4} };
...
void BinaryProcessor::CreateTestTextFileWithStrings(
    ...
    std::vector<std::string> test_lines = {
        "Привет мир.",        "Как дела?",            "Сегодня хорошая погода!",
        "Программирование это интересно.", "C++ мощный язык.",
        "Что нового?",        "Отличная работа!",     "Продолжаем учиться.",
        "Вопросы и ответы?",  "Конец задания." };
